//const { google } = require('googleapis');

import { google } from 'googleapis';
//const credentials = require('./credentials.json');
const keys = {
    client_email: "forward-gserviceaccount-com@forward-fuze-404507.iam.gserviceaccount.com",
    private_key: "-----BEGIN PRIVATE KEY-----\\nyVobFs7+DsHbb1T93FuvaUwg\n-----END PRIVATE KEY-----\n",
  };

const spreadsheetId = '1Pkd9pRR64oqXCrqcexndOmpcAKyjPzSHwExr4QRGrsY';
let googleSheet;

async function initClicent(){
  const googleClient = new google.auth.JWT(keys.client_email, null, keys.private_key, ['https://www.googleapis.com/auth/spreadsheets']);
  googleClient.authorize(function(err) {
    if (err) {
      console.error('Authorization failed:', err);
      callback(err);
      return;
    }
    googleSheet = google.sheets({ version: 'v4', auth: googleClient });
  })
}

function getGoogleSheet(){
  if(googleSheet){
    return googleSheet;
  }
  return null;
}


function createNewGoogleSheetWithName(sheetName){
    // Create a new spreadsheet
    googleSheet.spreadsheets.create({
      resource: {
        properties: {
          title: `Sheet_${sheetName}` // Set the title with the current date
        }
      }
    }, (err, response) => {
      if (err) {
        console.error('The API returned an error: ' + err);
        callback(err);
      } else {
        console.log('New spreadsheet created with the title: ' + response.data.properties.title);
        //callback(null, response.data);
      }
    });
}

async function addSheetToSpreadsheet(sheetName) {
  try {
    const sheetProperties = {
      title: sheetName, // Title for the new sheet
      gridProperties: {
        rowCount: 100, // Number of rows
        columnCount: 10, // Number of columns
      },
    };

    const addSheetRequest = {
      spreadsheetId,
      resource: {
        requests: [
          {
            addSheet: {
              properties: sheetProperties,
            },
          },
        ],
      },
    };

    const response = await googleSheet.spreadsheets.batchUpdate(addSheetRequest);
    console.log('New sheet added:', response.data.replies[0].addSheet.properties.title);
  } catch (error) {
    console.error('Error adding sheet:', error);
  }
}


function writeValue(cell, value){
  googleSheet.spreadsheets.values.update({
    spreadsheetId: spreadsheetId,
    range: cell,
    valueInputOption: 'RAW',
    resource: {
      values: [[value]],
    },
  }, (err, response) => {
    if (err) {
      console.error('The API returned an error: ' + err);
    } else {
      console.log('Value updated successfully!');
    }
  });
}


module.exports = { 
  initClicent,
  addSheetToSpreadsheet,
  writeValue
 };
